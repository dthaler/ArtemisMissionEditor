...
namespace ArtemisMissionEditor
{
    ...
    class _PanelSpaceMap : UserControl
	{
		...
		{
            return (double)_PSM_p_mapSurface.Width / (Space.MaxX + Space.PaddingX * 2.0);
		}

		public double GetMapScaleMin()
		{
			return (double)_PSM_p_mapSurface.Width / (Space.MaxX + Space.PaddingX * 2.0) < (double)_PSM_p_mapSurface.Height / (Space.MaxZ + Space.PaddingZ * 2.0) 
                ? (double)_PSM_p_mapSurface.Width / (Space.MaxX + Space.PaddingX * 2.0) 
                : (double)_PSM_p_mapSurface.Height / (Space.MaxZ + Space.PaddingZ * 2.0);
		}
		...
		private void AssignPropertyGrid_RecursivelyFindControls(Control c, string panelName)
		{
			foreach (Control item in c.Controls)
			{
				...
				AssignPropertyGrid_RecursivelyFindControls(item, panelName);
			}
		...
		public void AssignPropertyGrid(PropertyGrid value = null)
		{
			...
			if (_propertyEditorPG.Parent != null && _propertyEditorPG.Parent.Parent != null && _propertyEditorPG.Parent.Parent.GetType() == typeof(SplitContainer))
			{
				...
				AssignPropertyGrid_RecursivelyFindControls(_propertyContainer.Panel2, "Panel2");
				if (String.IsNullOrEmpty(_propertyContainerPossiblesPanel)) AssignPropertyGrid_RecursivelyFindControls(_propertyContainer.Panel1, "Panel1");
			}
		}
		...
		private int HowManyAsteroidsFitOnArc(double startRadius, double angle, int orbitID, ref double rem, ref double remLast, int max, int skip)
		{
            ...
            int result = (int)Math.Truncate(arcLength / Space.AsteroidWidth / 2.0);
		...
		{
            return (int)Math.Round(startRadius + ((orbitID / 2) * Math.Pow(-1, orbitID % 2)) * Space.AsteroidWidth * 2.0);
		}
		...
		public void Selection_Update()
		{
			if (_propertyEditorPG != null)
			{
                ...
                else
                {
                    ...
                    if (_propertyContainerPossiblesPanel == "Panel1") _propertyContainer.Panel1Collapsed = !MapObjectNamed.IsPropertyPresent("raceKeys", (MapObjectNamed[])_propertyEditorPG.SelectedObjects) || _propertyEditorPG.SelectedObjects.Count() == 0;
                    if (_propertyContainerPossiblesPanel == "Panel2") _propertyContainer.Panel2Collapsed = !MapObjectNamed.IsPropertyPresent("raceKeys", (MapObjectNamed[])_propertyEditorPG.SelectedObjects) || _propertyEditorPG.SelectedObjects.Count() == 0;
                }
				UpdatePossibleLists();
			}
			UpdateObjectsText();
        ...
        /// <summary>
        /// Update lists of possible vessels for the current selection.
        /// </summary>
		public void UpdatePossibleLists()
		{
            if (_possibleRacesLB == null && _possibleVesselsLB == null)
                return;
            ...
            //Prepare races list
            List<string> possibleRaceIDs = VesselData.Current.GetPossibleRaceIDs(mo.GetRaceKeysForXml());

            //Output races list
            if (_possibleRacesLB != null)
            {
                foreach (string item in possibleRaceIDs)
                    _possibleRacesLB.Items.Add(VesselData.Current.RaceToString(item));
            }

            if (_possibleVesselsLB != null)
            {
                //Prepare vessel list
                List<string> possibleVesselIDs = VesselData.Current.GetPossibleVesselIDs(possibleRaceIDs, mo.GetHullKeysForXml());

                //Finally output the vessel list
                foreach (string id in possibleVesselIDs)
                    _possibleVesselsLB.Items.Add(VesselData.Current.VesselToString(id));
            }
		}
        ...
        {
            if (_cursorCoordinates != null)
                _cursorCoordinates.Text = ((long)Space.MaxX -PointXToSpaceMap(_curPointX)).ToString() + ", " + PointZToSpaceMap(_curPointY).ToString();
        }
		...
		public void Reset()
		{
			...
			if (Settings.Current.DefaultSpaceMapScalePadded)
                coeff = (((double)_PSM_p_mapContainer.Width / Space.MaxX) < ((double)_PSM_p_mapContainer.Height /Space.MaxZ)) 
                    ? ((double)_PSM_p_mapContainer.Width / Space.MaxX) 
                    : ((double)_PSM_p_mapContainer.Height /Space.MaxZ);
			else
                coeff = (((double)_PSM_p_mapContainer.Width / (Space.MaxX + Space.PaddingX * 2)) < ((double)_PSM_p_mapContainer.Height / (Space.MaxZ + Space.PaddingZ * 2))
                    ? ((double)_PSM_p_mapContainer.Width / (Space.MaxX + Space.PaddingX * 2)) 
                    : ((double)_PSM_p_mapContainer.Height / (Space.MaxZ + Space.PaddingZ * 2)));

			_PSM_p_mapSurface.Width = (int)Math.Round((Space.MaxX + Space.PaddingX * 2.0) * coeff);
            _PSM_p_mapSurface.Height = (int)Math.Round((Space.MaxZ + Space.PaddingZ * 2.0) * coeff);
		...
		public void MoveBy(int amountX, int amountY)
		{
			...
			bool widthIsDeciding = (Space.MaxX / _PSM_p_mapContainer.Width) > (Space.MaxZ / _PSM_p_mapContainer.Height);
			if (widthIsDeciding)
			{
		...
		public void ScaleBy(int percent, double relative_x = 0.5, double relative_y = 0.5)
		{
			...
			if (widthIsDeciding)
			{
				...
				zoomChangeZ = (int)Math.Round((double)zoomChangeX / Space.MaxX * Space.MaxZ);

				//Apply changes
				_Width += zoomChangeX;
				_Height = (int)Math.Round((double)_Width / Space.MaxX * Space.MaxZ); //this is safer
			...
			else
			{
				...
				//Calculate other dimension's value for purpose of adjusting the topleft corner
				zoomChangeX = (int)Math.Round((double)zoomChangeZ / Space.MaxZ * Space.MaxX);

				//Apply changes
				_Height += zoomChangeZ;
				_Width = (int)Math.Round((double)_Height / Space.MaxZ * Space.MaxX); //this is safer
				_Left -= (int)Math.Round(zoomChangeX * relative_x);
				_Top -= (int)Math.Round(zoomChangeZ * relative_y);
		...
		public void ExecuteCommand_AddObjectToMap(string type, bool additive)
		{
			object newObject = SpaceMap.AddObject(type, PointXToSpaceMap(_curPointX), 0,
													PointZToSpaceMap(_curPointY), 0, false);
			if (newObject.GetType() != typeof(MapObjectNameless))
			{
				MapObjectNamed mo = (MapObjectNamed)newObject;
                mo.Copy(true, SpaceMap.GetSelectionNamed());
				if (!additive)
			...
			{
				MapObjectNameless nmo = (MapObjectNameless)newObject;
				if (SpaceMap.SelectionNameless != null)
		...
		private void InitializeComponent()
		{
            ...
            this._PSM_tb_scapeGoat.Location = new System.Drawing.Point(295, 454);
		...
		public void Redo(SpaceSavedState state = null)
		{
		...
		private bool CopyDo()
		{
			...
			if (SpaceMap.SelectionNameless != null)
				...
				foreach (MapObjectNamed item in SpaceMap.NamedObjects)
					if (item.Selected)
		...
		private int PasteDo()
		{
			...
			List<MapObjectNamed> namedObjects = new List<MapObjectNamed>();
			List<MapObjectNameless> namelessObjects = new List<MapObjectNameless>();

			foreach (XmlNode childNode in root)
			{
				MapObjectNamed mo = MapObjectNamed.NewFromXml(childNode);
				MapObjectNameless nmo = MapObjectNameless.NewFromXml(childNode);
